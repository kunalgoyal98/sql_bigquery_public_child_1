...
SQLStatement_1 AS (
  ...
  WHERE c_base_dependency_macro IS NOT NULL
        or c_boolean_macro IN (true, false, NULL)
        or c_numeric_1 > -100
        or c_int64 BETWEEN 100
        and 10000000
...
Aggregate_1 AS (
  ...
  SELECT 
    ...
    - BIT_OR(c_int64)
    + BIT_XOR(c_int64)
    + COUNT(DISTINCT c_bignumeric) AS c_int_aggregate,
...
Join_1 AS (
  ...
  RIGHT JOIN Limit_4 AS env_uitesting_shared_mid_model_1
     ON env_uitesting_shared_parent_model_1.c_int64 = env_uitesting_shared_mid_model_1.c_int64
    and env_uitesting_shared_parent_model_1.c_bignumeric = env_uitesting_shared_mid_model_1.c_bignumeric
    and env_uitesting_shared_parent_model_1.c_bool = env_uitesting_shared_mid_model_1.c_bool
...
AllStunningOne AS (
  ...
  SELECT 
    ...
    or (10 <> 10)
    or (20 = 10)
    or (20 BETWEEN 10 and 21)
    or ('hello' LIKE '%h\\%\\_\\/%')
    or (10 IN (20, 10, 30))
    or ((20 >= 30) IS TRUE)
    or ((20 >= 30) IS NOT TRUE)
    or ((20 >= 30) IS FALSE)
    or ((20 >= 30) IS NOT FALSE)
    or ((10 > 20) IS UNKNOWN)
    or ((10 > 20) IS NOT UNKNOWN)
    or ((10 > 20) IS NOT NULL)
    or ((10 > 20) IS NULL)
    or (COLLATE('Foo', 'und:ci') LIKE COLLATE('%foo%', 'und:ci'))
    or ('MASSE' LIKE 'Ma?e')
    or ('\u3042' LIKE '%\u30A2%')
    or (
         CASE
    ...
    or (IFNULL(NULL, 0) = 0)
    or (NULLIF(10, 0) = 10)
    or ((ANY_VALUE(c_int64) OVER (ORDER BY LENGTH(c_string) ROWS BETWEEN 1 PRECEDING AND CURRENT ROW)) = 0)
    or (CAST('1' AS string) = '1')
    or (CAST('1' AS string) = '1')
    or (CAST('1' AS BIGNUMERIC) = 1)
    or (CAST('TRUE' AS BOOL) = TRUE)
    or (CAST(CAST('hi' AS BYTES) AS STRING) = 'hi')
    or (CAST('2021-04-20' AS DATE) = DATE'2020-09-22')
    or (CAST('2021-04-20 00:00:00' AS TIMESTAMP) = TIMESTAMP'2020-09-22 00:00:00')
    or (CAST('23.45' AS FLOAT64) = 23.45)
    or (CAST('-23' AS INT64) = -23)
    or (CAST('-0x123' AS INT64) = -291)
    or (CAST('123' AS NUMERIC) = 123)
    or (CAST(b'\x48\x65\x6c\x6c\x6f' AS STRING FORMAT 'ASCII') = 'Hello')
    or (CAST(CURRENT_DATE() AS STRING) = '2020-12-12')
    or (CAST(CURRENT_DATE() AS STRING FORMAT 'DAY') = 'MONDAY')
    or (CAST(TIMESTAMP'2008-12-25 00:00:00+00:00' AS STRING FORMAT 'YYYY-MM-DD HH24:MI:SS TZH:TZM') = '2008-12-25 00:00:00+00:00')
    or (CAST(TIMESTAMP'2008-12-25 00:00:00+00:00' AS STRING FORMAT 'YYYY-MM-DD HH24:MI:SS TZH:TZM' AT TIME ZONE 'Asia/Kolkata') = '2008-12-25 00:00:00+00:00')
    or ((CAST('18-12-03' AS DATE FORMAT 'YY-MM-DD') = CAST('18-12-03' AS DATE FORMAT 'YY-MM-DD')))
    or (CAST('DEC 03, 2018' AS DATE FORMAT 'MON DD, YYYY') = CAST('DEC 03, 2018' AS DATE FORMAT 'MON DD, YYYY'))
    or (CAST('15:30' AS TIME FORMAT 'HH24:MI') = CAST('15:30' AS TIME FORMAT 'HH24:MI'))
    or (CAST('03:30 P.M.' AS TIME FORMAT 'HH:MI P.M.') = CAST('03:30 P.M.' AS TIME FORMAT 'HH:MI P.M.'))
    or (CAST('03:30 P.M.' AS TIME FORMAT 'HH:MI A.M.') = CAST('03:30 P.M.' AS TIME FORMAT 'HH:MI A.M.'))
    or (CAST(-12345.678 AS STRING FORMAT '$999,999.999') = '-$12,345.678')
    or (CAST(-123456 AS STRING FORMAT '9.999EEEE') = '-1.235E+05')
    or (PARSE_BIGNUMERIC("  -  123.45 ") = 123.45)
    or (PARSE_BIGNUMERIC("  1,2,,3,.45 + ") = 123.25)
    or (PARSE_NUMERIC("12.34e-1-") = -1.234)
    or (SAFE_CAST("apple" AS INT64) IS NULL)
    or (ARRAY_TO_STRING(['asd', 'asd'], '--') = 'asd--asd')
    or (TO_BASE64(b'\377\340') IS NULL)
    or (SAFE_CONVERT_BYTES_TO_STRING(b'\xc2') IS NULL)
    or is_inf(CAST('Infinity' AS float64))
    or is_nan(CAST('NaN' AS float64))
    or (CONTAINS_SUBSTR('the blue house', 'Blue house'))
    or (CONTAINS_SUBSTR((23, 35, 41), '35'))
    or (CONTAINS_SUBSTR(('abc', ['def', 'ghi', 'jkl'], 'mno'), 'jk'))
    or (CONTAINS_SUBSTR(JSON '{"lunch":"soup"}', "lunch", json_scope => "JSON_VALUES"))
    or (CONTAINS_SUBSTR(JSON '{"lunch":"soup"}', "lunch", json_scope => "JSON_KEYS_AND_VALUES"))
    or (ENDS_WITH(c_string, 'e'))
    or (REGEXP_CONTAINS('abhishek@prophecy.io', r'@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+'))
    or (REGEXP_CONTAINS('abhishek@prophecy.io', r'^([\w.+-]+@foo\.com|[\w.+-]+@bar\.org)$'))
    or (STARTS_WITH('asdasdasd', 'b'))
    or (BOOL(JSON_QUERY(JSON '{"hotel class": "5-star", "vacancy": true}', "$.vacancy")))
    or (SAFE.FLOAT64(JSON '"strawberry"') IS NULL) AS c_bool_expr,
    ...
    + log(100, 10)
    + log10(10)
    + GREATEST(1, 2, 3, 4, 5.23, 0, -10)
    + LEAST(1, 2, 3, 4, 5.23, 0, -10)
    + div(10, 10)
    + SAFE_DIVIDE(0, 20)
    + SAFE_MULTIPLY(1, 2)
    + SAFE_NEGATE(2000)
    + SAFE_ADD(1, 2)
    + SAFE_SUBTRACT(10, 20000)
    + mod(10, 2)
    + ROUND(2.8)
    + ROUND(-2.3)
    ...
    + RANGE_BUCKET('a', ['a', 'b', 'c', 'd'])
    + BYTE_LENGTH('?????')
    + CHAR_LENGTH('asdasd?????')
    + CHARACTER_LENGTH('asdasd?????')
    + INSTR('banana', 'an', 1, 2)
    + LENGTH('?????')
    + OCTET_LENGTH('sadasd')
    + REGEXP_INSTR('abhisheks@gmail.com', '@[^.]*')
    + STRPOS('abhishek@gmail.com', '@')
    + UNICODE('?')
    + INT64(JSON_QUERY(JSON '{"gate": "A4", "flight_number": 2}', "$.flight_number"))
    + FLOAT64(JSON_QUERY(JSON '{"vo2_max": 2.1, "age": 18}', "$.vo2_max"))
    + ARRAY_LENGTH([1, 2])
    + EXTRACT(HOUR FROM DATETIME(2008, 12, 25, 15, 30, 0))
    - EXTRACT(WEEK(SUNDAY) FROM DATETIME(TIMESTAMP'2017-11-05 00:00:00+00', "UTC"))
    + EXTRACT(HOUR FROM TIME "15:30:00")
    - EXTRACT(MONTH FROM (INTERVAL '1-2 3 4:5:6.789999' YEAR TO SECOND)) AS c_add_expr,
...
combine_multi_tables_1 AS (
  ...
  {{ SQL_BigquerySharedBasic.combine_multi_tables(table_1 = 'Filter_1', table_2 = 'Limit_2', table_3 = 'OrderBy_1', table_4 = 'Limit_1', table_5 = 'Limit_6', col_table_1 = 'c_numeric_1') }}
